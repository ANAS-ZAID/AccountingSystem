...
{
    public partial class MovementOfItems : Form
    {
        MovementOfItemsController controller;
        public MovementOfItems()
        {
            InitializeComponent();
            controller = new MovementOfItemsController();
        }
        private void MovementOfItems_Load(object sender, EventArgs e)
        {
        ...
        void lodeData()
        {
            ...
            dataSetMovementOfItemsBindingSource.DataSource = controller.dataSource.DataSource;
            //   reportViewer1.LocalReport.SetParameters(new ReportParameter("allAccounts", (controller.account.id == 0).ToString()));
        //    reportViewer1.LocalReport.SetParameters(new ReportParameter("showOrHidePreviousBalance", (controller.startDate == null).ToString()));
