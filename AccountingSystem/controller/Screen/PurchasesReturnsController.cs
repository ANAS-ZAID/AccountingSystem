...
{
    public class PurchasesReturnsController
    : DBContextController
    {
        public DateTime? startDate;
        public DateTime? endDate;
        public AppTable detailTable;
        List<PurchaseDetail> newDetails;
        ...
        public Purchase temp;
        public Purchase tempSearch;
        List<JournalEntry> tempJournalEntries;
        public DateTime? lastDate;
        int newNumber;
        public bool isWholesale => temp.priceType == PriceType.جمله.ToString();

        public PurchasesReturnsController(bool IsAddReturn = false)
        {
            permissions = model.LoginData.permissions["purchasesReturns"];
            columnsNamesInAR = new List<string>() { "الرقم", "رقم الفاتوره ", "نوع الدفع", "التأريخ", "المخزن", "الصندوق", "المورد", "المبلغ", "العمله", "المبلغ المدفوع", "المتبقي", "الخصم", "نوع السعر", "تأريخ الإضافه" };
            transactionType = TransactionType.مرتجع_مشتريات.ToString();
            detailTable = new AppTable();
            dataSource.DataSource = typeof(Purchase);
        ...
        protected void fillDetailTable()
        {
            ...
            foreach (var detail in temp?.PurchaseDetails.Primary())
            {
        ...
        bool newRow(PurchaseDetail detail)
        {
        ...
        internal AppRow newRow(Classify item)
        {
            ...
            new AppCell() {value="1" },
            new AppCell() {value=(isWholesale?measurement.WholesalePurchasePrice:measurement.purchasePrice).Format() },
            new AppCell() {value=(isWholesale?measurement.WholesalePurchasePrice:measurement.purchasePrice).Format() },
            new AppCell() {},
            }
            };
        ...
        public override void clearTempData()
        {
            ...
            temp = new Purchase()
            {
                ...
                Cashier = cashiers.FirstOrDefault(),
        ...
        public void clearHomeTempData()
        {
            ...
            tempSearch = new Purchase() { Cashier = null, Supplier = null, Store = null, Employee = null, Currency = null };
        }
        bool IsAddReturn = false;
        public bool dataProcessing(string number, string exchangeRate, string amountPaid, AppTable data)
        {
            ...
            if (!ValidatingData.validatingData(temp.Supplier, "المورد", false))
                return false;
            ...
            temp.supplierId = temp.Supplier.id;
            ...
            tempJournalEntries = temp.AddJournalEntry(newDetails, newNumber, true);
            if (IsAddReturn)
                return addReturn();
            if (IsAdd)
                return add();
        ...
        private bool addReturn()
        {
            ...
            using (var transaction = dBContext.Database.BeginTransaction())
            {
                ...
                try
                {
                    var any = dBContext.Purchases?.FirstOrDefault(x => x.number == newNumber);
                    if (any != null)
                    {
                        AppDialogAleart.showAleartPreExistingData("توجد فاتوره سابقه بهذا الرقم");
                        return false;
                    }
                    any = dBContext.Purchases?.FirstOrDefault(x => x.originalInvoiceId == temp.id);
                    if (any != null)
                    {
                    ...
                    Purchase purchase = temp.Copy(newDetails);
                    purchase.number=newNumber;
                    purchase.originalInvoiceId = temp.id;
        ...
        public AppTable newDetailTable
        {
            set
            {
                newDetails = new List<PurchaseDetail>();
                for (int i = 0; i < value.Rows.Count; i++)
                {
                    ...
                    if (item != null && item.id != 0)
                    {
                        ...
                        var data = new PurchaseDetail() { itemId = item.id, measurementItemId = measurementItemId, unitPrice = unitPrice, quantity = quantity, description = description };
        ...
        void addCompositeItems(int measurementItemId, decimal quantity)
        {
            ...
            {
                newDetails.Add(new PurchaseDetail()
                {
        ...
        public override bool update()
        {
            ...
            using (var transaction = dBContext.Database.BeginTransaction())
            {
                ...
                try
                {
                    ...
                    var any = dBContext.Purchases?.FirstOrDefault(x => x.number == newNumber && x.id != temp.id);
                    if (any != null)
                    {
                        AppDialogAleart.showAleartPreExistingData("توجد فاتوره سابقه بهذا الرقم");
                        return false;
                    ...
                    dBContext.JournalEntries.AddRange(tempJournalEntries);
                    dBContext.SaveChanges();
                    dBContext.PurchaseDetails.RemoveRange(temp.PurchaseDetails);
                    dBContext.SaveChanges();
                    temp.PurchaseDetails = newDetails;
                    temp.number = newNumber;
                    dBContext.PurchaseDetails.AddOrUpdate(temp.PurchaseDetails.ToArray());
                    dBContext.SaveChanges();
                    transaction.Commit();
                    status = true;
                    AppDialogAleart.showAleartSuccess();
                    ...
                    using (var transaction = dBContext.Database.BeginTransaction())
                    {
                        try
                        {
                            foreach (var id in keys)
                            {
                                ...
                                dBContext.PurchaseDetails.RemoveRange(temp.PurchaseDetails);
                                dBContext.Purchases.Remove(temp);
        ...
        public void selectSupplier(object supplier)
        {
            if (HasAddAndUpdateScreenDataProcessed)
            {
                temp.Supplier = (Supplier)supplier ?? null;
                temp.supplierId = temp.Supplier?.id;
            }
            else if (HasHomeScreenDataProcessed)
            {
                tempSearch.Supplier = (Supplier)supplier ?? null;
                tempSearch.supplierId = tempSearch.Supplier?.id;
            }
        ...
        public void selectPriceType(object value)
        {
            if (HasAddAndUpdateScreenDataProcessed)
            {
        ...
        public override bool find(int id)
        {
            ...
            try
            {
                ...
                temp = dBContext.Purchases.Returned(IsAddReturn)?.FirstOrDefault(i => i.id == id);
                if (temp == null)
                    throw new Exception();
        ...
        public override bool lodeData(bool shearch = false, dynamic additional = null)
        {
            if (permissions.viewPermission ?? false)
            {
                ...
                try
                {
                    ...
                    var allData = dBContext.Purchases.Returned(true).ToList();
                    if (shearch)
        ...
        public override void showDialogAdd()
        {
            if (permissions.addPermission.Value)
            {
                ...
                DialogAddAndUpdatePurchasesReturns dialog = new DialogAddAndUpdatePurchasesReturns(this);
                dialog.ShowDialog();
                endHADSDP();
            }
            else AppDialogAleart.showAleartNoPermissions();
        ...
        public void print(int id)
        {
            ...
            dynamic bill = new { };
                ...
                detail = temp.PurchaseDetails.Where(x => x.type != MeasurementsItemType.مركب.ToString()).Select(d => new DataSetBills() { name = d.item.nameAr, unitName = d.MeasurementsItem.Unit.name, quantity = d.quantity ?? 0, unitPrice = d.unitPrice ?? 0, description = d.description }).ToList();
                bill = new
                {
                    name = "المورد: " + temp.Supplier.name,
                    number = temp.number.Value.ToString(),
                    date = temp.date.Value.ToString(SharedData.formatDisplayDate),
                    store = temp.Store.name,
                    type = " مردود مشتريات (" + temp.paymentType + ")",
                    currencyName = temp.Currency.name,
                    currencyCode = temp.Currency.code,
                    amountPaid = (temp.amountPaid ?? 0).ToString(),
                    total = (temp.PurchaseDetails.ToList().TotalPrice()).ToString(),
